dictionary_version = ["Olympia G4", "Dictionary of turn #1"]

# there is still the problem with "wood" beeing converted to 77, while still beeing a skill. use COLLECT!

# need to update the whole dict once in a while.. nobles, etc.
# also, 401, 402,403 have been added and not added to the main dictionary yet.

aliaslist = ['accept', 'admit', 'attack', 'banner', 'behind', 'board', 'bribe', 'breed', 'build', 'buy', 'catch', 'claim', 'collect', 'contact', 'decree', 'default', 'defend', 'die', 'drop', 'email', 'execute', 'exhume', 'explore', 'fee', 'ferry', 'fish', 'flag', 'fly', 'forget', 'form', 'format', 'garrison', 'get', 'give', 'guard', 'honor', 'hostile', 'improve', 'lore', 'make', 'message', 'move', 'name', 'neutral', 'notab', 'oath', 'password', 'pay', 'pillage', 'players', 'pledge', 'post', 'press', 'promote', 'public', 'quarry', 'quest', 'quit', 'raze', 'recruit', 'repair', 'research', 'resend', 'rumor', 'sail', 'seek', 'sell', 'sneak', 'stack', 'stop', 'study', 'surrender', 'take', 'terrorize', 'train', 'ungarrison', 'unload', 'unstack', 'use', 'vis_email', 'wait', 'clear', 'day', 'turn', 'ferry', 'flag', 'fog', 'item', 'loc', 'not', 'owner', 'rain', 'ship', 'shiploc', 'stack', 'time', 'top', 'unit', 'wind', 'n', 'north', 's', 'south', 'w', 'west', 'e', 'east', 'in', 'out', 'all', 'gold', 'peasant', 'worker', 'soldier', 'archer', 'knight', 'elite_guard', 'pikeman', 'blessed_soldier', 'ghost_warrior', 'sailor', 'swordsman', 'crossbowman', 'elite_archer', 'angry_peasant', 'pirate', 'elf', 'spirit', 'undead', 'savage', 'skeleton', 'barbarian', 'wild_horse', 'riding_horse', 'warmount', 'winged_horse', 'nazgul', 'flotsam', 'battering_ram', 'catapult', 'siege_tower', 'ratspider_venom', 'lana_bark', 'avinia_leaf', 'spiny_root', 'farrenstone', 'yew', 'elfstone', 'mallorn_wood', 'pretus_bones', 'longbow', 'plate_armor', 'longsword', 'pike', 'ox', 'wood', 'stone', 'iron', 'leather', 'ratspider', 'mithril', 'gate_crystal', 'blank_scroll', 'crossbow', 'fish', 'opium', 'woven_basket', 'clay_pot', 'tax_cookie', 'drum', 'hide', 'mob_cookie', 'lead', 'pitch', 'centaur', 'minotaur', 'undead_cookie', 'fog_cookie', 'wind_cookie', 'rain_cookie', 'mage_menial_cookie', 'giant_spider', 'rat', 'lion', 'giant_bird', 'giant_lizard', 'bandit', 'chimera', 'harpie', 'dragon', 'orc', 'gorgon', 'wolf', 'crystal_orb', 'cyclops', 'giant', 'faery', 'petty_thief_cookie', 'hound', 'imperial_throne', 'crown_of_prosperity', 'skull_of_bastrestric', 'shipcraft', 'sailing', 'shipbuilding', 'fishing', 'combat', 'survive_fatal_wound', 'fight_to_the_death', 'construct_catapult', 'defense', 'archery', 'swordplay', 'weaponsmithing', 'stealth', 'petty_thievery', 'determine_inventory_of_character', 'determine_skills_of_character', "determine_character's_lord", 'conceal_faction', 'learn_of_richest_nearby_noble', 'torture_prisoner', 'sneak_into_structure', 'conceal_self', 'beastmastery', 'bird_spy', 'capture_beasts_in_battle', 'use_beasts_in_battle', 'breed_beasts', 'catch_wild_horses', 'train_wild_horse_to_riding_horse', 'train_wild_horse_to_warmount', 'summon_wild_men', 'persuade_wild_men_to_remain', 'breed_hound', 'persuasion', 'bribe_noble', 'persuade_oathbound_noble', 'raise_peasant_mob', 'rally_peasant_mob', 'incite_mob_violence', 'train_angry_peasants', 'construction', 'construct_siege_tower', 'stone_quarrying', 'alchemy', 'extract_venom_from_ratspider', 'make_potion_of_slavery', 'brew_healing_potion', 'brew_potion_of_death', 'record_skill_on_scroll', 'collect_rare_elements', 'turn_lead_into_gold', 'forestry', 'construct_battering_ram', 'harvest_lumber', 'harvest_yew', 'collect_rare_foliage', 'harvest_mallorn_wood', 'harvest_opium', 'improve_opium_production', 'mining', 'mine_iron', 'mine_gold', 'mine_mithril', 'trade', 'conceal_identity_of_trader', 'find_tradegood_for_sale', 'find_market_for_tradegood', 'religion', 'receive_vision', 'resurrect_dead_noble', 'preparatory_ritual', 'lay_to_rest', 'remove_blessing_from_soldiers', 'immunity_from_vision', 'magic', 'meditate', 'perform_common_tasks_for_gold', 'appear_common', 'view_current_aura_level_of_others', 'reveal_abilities_of_another_mage', 'tap_health_for_aura', 'heal', 'modern_magic_script', 'shroud_abilities_from_scry', 'detect_ability_scry', 'dispel_ability_shroud', 'advanced_meditation', 'hinder_meditation', 'weather_magic', 'fierce_wind', 'bind_storm_to_ship', 'scribe_weather_symbols', 'summon_rain', 'summon_wind', 'summon_fog', 'direct_storm', 'dissipate_storm', 'renew_storm_strength', 'lightning_bolt', 'seize_control_of_storm', 'fog_of_death', 'scrying', 'scry_location', 'shroud_location_from_magical_scry', 'dispel_location_shroud', 'ciphered_writing_of_areth-pirn', 'create_magical_barrier', 'remove_magical_barrier', 'locate_character', 'detect_location_scry', 'farcasting', 'save_farcast_state', 'banish_undead', 'gatecraft', 'teleport', 'detect_gates', 'jump_through_gate', 'seal_gate', 'unseal_gate_with_key', 'notify_if_gate_unsealed', 'forcefully_unseal_gate', 'reveal_gate_key', 'notify_of_gate_jumps', 'language_of_the_ancients', 'reverse_jump_through_gate', 'artifact_construction', 'forge_auraculum', 'reveal_creator_of_artifact', 'reveal_where_artifact_was_created', 'cloak_creator_of_artifact', 'cloak_region_of_artifact_creation', 'curse_noncreator_loyalty', 'forge_magical_weapon', 'forge_magical_armor', 'forge_magical_bow', 'dispel_cloaking_from_artifact', 'arcane_symbols', 'destroy_artifact', 'forge_palantir', 'necromancy', 'transcend_death', 'runes_of_evil', 'summon_ghost_warriors', 'raise_undead', 'summon_demon_lord', 'renew_demon_bond', 'banish_demon_lord', 'eating_of_the_dead', 'aura_blast', 'absorb_aura_blast', 'advanced_sorcery', 'trance', 'teleport_items']


reprocessordict = {
    "0":["all","item"],
    "1":["gold","item"],
    "10":["peasant","item"],
    "11":["worker","item"],
    "12":["soldier","item"],
    "13":["archer","item"],
    "14":["knight","item"],
    "15":["elite_guard","item"],
    "16":["pikeman","item"],
    "17":["blessed_soldier","item"],
    "18":["ghost_warrior","item"],
    "19":["sailor","item"],
    "20":["swordsman","item"],
    "21":["crossbowman","item"],
    "22":["elite_archer","item"],
    "23":["angry_peasant","item"],
    "24":["pirate","item"],
    "25":["elf","item"],
    "26":["spirit","item"],
    "31":["undead","item"],
    "32":["savage","item"],
    "33":["skeleton","item"],
    "34":["barbarian","item"],
    "51":["wild_horse","item"],
    "52":["riding_horse","item"],
    "53":["warmount","item"],
    "54":["winged_horse","item"],
    "55":["nazgul","item"],
    "59":["flotsam","item"],
    "60":["battering_ram","item"],
    "61":["catapult","item"],
    "62":["siege_tower","item"],
    "63":["ratspider_venom","item"],
    "64":["lana_bark","item"],
    "65":["avinia_leaf","item"],
    "66":["spiny_root","item"],
    "67":["farrenstone","item"],
    "68":["yew","item"],
    "69":["elfstone","item"],
    "70":["mallorn_wood","item"],
    "71":["pretus_bones","item"],
    "72":["longbow","item"],
    "73":["plate_armor","item"],
    "74":["longsword","item"],
    "75":["pike","item"],
    "76":["ox","item"],
    "77":["wood","item"],
    "78":["stone","item"],
    "79":["iron","item"],
    "80":["leather","item"],
    "81":["ratspider","item"],
    "82":["mithril","item"],
    "83":["gate_crystal","item"],
    "84":["blank_scroll","item"],
    "85":["crossbow","item"],
    "87":["fish","item"],
    "93":["opium","item"],
    "94":["woven_basket","item"],
    "95":["clay_pot","item"],
    "96":["tax_cookie","item"],
    "98":["drum","item"],
    "99":["hide","item"],
    "101":["mob_cookie","item"],
    "102":["lead","item"],
    "261":["pitch","item"],
    "271":["centaur","item"],
    "272":["minotaur","item"],
    "273":["undead_cookie","item"],
    "274":["fog_cookie","item"],
    "275":["wind_cookie","item"],
    "276":["rain_cookie","item"],
    "277":["mage_menial_cookie","item"],
    "278":["giant_spider","item"],
    "279":["rat","item"],
    "280":["lion","item"],
    "281":["giant_bird","item"],
    "282":["giant_lizard","item"],
    "283":["bandit","item"],
    "284":["chimera","item"],
    "285":["harpie","item"],
    "286":["dragon","item"],
    "287":["orc","item"],
    "288":["gorgon","item"],
    "289":["wolf","item"],
    "290":["crystal_orb","item"],
    "291":["cyclops","item"],
    "292":["giant","item"],
    "293":["faery","item"],
    "294":["petty_thief_cookie","item"],
    "295":["hound","item"],
    "401":["imperial_throne","item"],
    "402":["crown_of_prosperity","item"],
    "403":["skull_of_bastrestric","item"],

    # skills   
    "600":["shipcraft","skill"],
    "601":["sailing","skill"],
    "602":["shipbuilding","skill"],
    "603":["fishing","skill"],
    "610":["combat","skill"],
    "611":["survive_fatal_wound","skill"],
    "612":["fight_to_the_death","skill"],
    "613":["construct_catapult","skill"],
    "614":["defense","skill"],
    "615":["archery","skill"],
    "616":["swordplay","skill"],
    "617":["weaponsmithing","skill"],
    "630":["stealth","skill"],
    "631":["petty_thievery","skill"],
    "632":["determine_inventory_of_character","skill"],
    "633":["determine_skills_of_character","skill"],
    "634":["determine_character's_lord","skill"],
    "635":["conceal_faction","skill"],
    "636":["learn_of_richest_nearby_noble","skill"],
    "637":["torture_prisoner","skill"],
    "638":["conceal_self","skill"], # adapted for G4
    "639":["sneak_into_structure","skill"],
    "650":["beastmastery","skill"],
    "651":["bird_spy","skill"],
    "652":["capture_beasts_in_battle","skill"],
    "653":["use_beasts_in_battle","skill"],
    "654":["breed_beasts","skill"],
    "655":["catch_wild_horses","skill"],
    "656":["train_wild_horse_to_riding_horse","skill"],
    "657":["train_wild_horse_to_warmount","skill"],
    "658":["summon_wild_men","skill"],
    "659":["persuade_wild_men_to_remain","skill"],
    "661":["breed_hound","skill"],
    "670":["persuasion","skill"],
    "671":["bribe_noble","skill"],
    "672":["persuade_oathbound_noble","skill"],
    "673":["raise_peasant_mob","skill"],
    "674":["rally_peasant_mob","skill"],
    "675":["incite_mob_violence","skill"],
    "676":["train_angry_peasants","skill"],
    "680":["construction","skill"],
    "681":["construct_siege_tower","skill"],
    "682":["stone_quarrying","skill"],
    "690":["alchemy","skill"],
    "691":["brew_healing_potion","skill"], # adapted for G4.
    "692":["record_skill_on_scroll","skill"], # adapted for G4.
    "693":["extract_venom_from_ratspider","skill"],# adapted for G4.
    "694":["make_potion_of_slavery","skill"], # adapted for G4.
    "695":["collect_rare_elements","skill"], # adapted for G4.
    "696":["brew_potion_of_death","skill"], # adapted for G4.
    "697":["turn_lead_into_gold","skill"], # adapted for G4.
    "700":["forestry","skill"],
    "701":["construct_battering_ram","skill"],
    "702":["harvest_lumber","skill"],
    "703":["harvest_yew","skill"],
    "704":["collect_rare_foliage","skill"],
    "705":["harvest_mallorn_wood","skill"],
    "706":["harvest_opium","skill"],
    "707":["improve_opium_production","skill"],
    "720":["mining","skill"],
    "721":["mine_iron","skill"],
    "722":["mine_gold","skill"],
    "723":["mine_mithril","skill"],
    "730":["trade","skill"],
    "731":["conceal_identity_of_trader","skill"],
    "732":["find_tradegood_for_sale","skill"],
    "733":["find_market_for_tradegood","skill"],
    "750":["religion","skill"],
    "751":["receive_vision","skill"],
    "752":["lay_to_rest","skill"], # adapted for G4.
    "753":["preparatory_ritual","skill"], 
    "754":["resurrect_dead_noble","skill"], # adapted for G4.
    "755":["remove_blessing_from_soldiers","skill"],
    "756":["immunity_from_vision","skill"],
    "800":["magic","skill"],
    "801":["meditate","skill"],
    "802":["perform_common_tasks_for_gold","skill"],
    "803":["appear_common","skill"],
    "804":["view_current_aura_level_of_others","skill"],
    "805":["heal","skill"], # adapted for G4.
    "806":["modern_magic_script","skill"], # adapted for G4.
    "807":["reveal_abilities_of_another_mage","skill"],# adapted for G4.
    "808":["tap_health_for_aura","skill"],# adapted for G4.
    "809":["shroud_abilities_from_scry","skill"],
    "811":["detect_ability_scry","skill"],
    "812":["dispel_ability_shroud","skill"],
    "813":["advanced_meditation","skill"],
    "814":["hinder_meditation","skill"],
    "820":["weather_magic","skill"],
    "821":["fierce_wind","skill"],
    "822":["bind_storm_to_ship","skill"],
    "823":["scribe_weather_symbols","skill"],
    "824":["summon_wind","skill"], # adapted for G4.
    "825":["summon_rain","skill"], # adapted for G4.
    "826":["summon_fog","skill"],
    "827":["direct_storm","skill"],
    "828":["dissipate_storm","skill"],
    "829":["renew_storm_strength","skill"],
    "831":["lightning_bolt","skill"],
    "832":["seize_control_of_storm","skill"],
    "833":["fog_of_death","skill"],
    "840":["scrying","skill"],
    "841":["scry_location","skill"],
    "842":["ciphered_writing_of_areth-pirn","skill"], # adapted for G4.
    "843":["shroud_location_from_magical_scry","skill"],# adapted for G4.
    "844":["dispel_location_shroud","skill"], # adapted for G4.
    "845":["create_magical_barrier","skill"],
    "846":["remove_magical_barrier","skill"],
    "847":["locate_character","skill"],
    "848":["detect_location_scry","skill"],
    "849":["farcasting","skill"],
    "851":["save_farcast_state","skill"],
    "852":["banish_undead","skill"],
    "860":["gatecraft","skill"],
    "861":["detect_gates","skill"], # adapted for G4.
    "862":["jump_through_gate","skill"],# adapted for G4.
    "863":["language_of_the_ancients","skill"],
    "864":["seal_gate","skill"],
    "865":["unseal_gate_with_key","skill"],
    "866":["notify_if_gate_unsealed","skill"],
    "867":["forcefully_unseal_gate","skill"],
    "868":["reveal_gate_key","skill"],
    "869":["notify_of_gate_jumps","skill"],
    "871":["teleport","skill"], # adapted for G4.
    "872":["reverse_jump_through_gate","skill"],
    "880":["artifact_construction","skill"],
    "881":["forge_auraculum","skill"], # adapted for G4.
    "882":["arcane_symbols","skill"], # adapted for G4.
    "883":["forge_magical_weapon","skill"], # adapted for G4.
    "884":["forge_magical_armor","skill"], # adapted for G4.
    "885":["forge_magical_bow","skill"], # adapted for G4.
    "886":["curse_noncreator_loyalty","skill"], # adapted for G4.
    "887":["reveal_creator_of_artifact","skill"], # adapted for G4.
    "888":["reveal_where_artifact_was_created","skill"],
    "889":["destroy_artifact","skill"], # adapted for G4.
    "891":["cloak_creator_of_artifact","skill"], # adapted for G4.
    "892":["cloak_region_of_artifact_creation","skill"], # adapted for G4.
    "893":["dispel_cloaking_from_artifact","skill"],
    "894":["forge_palantir","skill"],
    "900":["necromancy","skill"],
    "901":["raise_undead","skill"], # adapted for G4.
    "902":["summon_ghost_warriors","skill"], # adapted for G4.
    "903":["runes_of_evil","skill"], # adapted for G4.
    "904":["summon_demon_lord","skill"], # adapted for G4.
    "905":["renew_demon_bond","skill"], # adapted for G4.
    "906":["banish_demon_lord","skill"], # adapted for G4.
    "907":["eating_of_the_dead","skill"], # adapted for G4.
    "908":["aura_blast","skill"], # adapted for G4.
    "919":["absorb_aura_blast","skill"],     # adapted for G4.
    "901":["transcend_death","skill"], # adapted for G4.
    "920":["advanced_sorcery","skill"],
    "921":["trance","skill"],
    "922":["teleport_items","skill"]
        }


preprocessordict = {
    # locations (added manually)
    # ---------
    
    # e.g.: "_Ivory_Tower":["8276","location"],

    # ALLIANCE NOBLES
    # ---------------

    # function cut out for G4, as alliance scripts are not included.
    # nobles can still be added manually.

    # e.g.: "Yoshi_the_Gulper":["6406","noble"],
    
    # END OF ALLIANCE NOBLES

    
    #items
    #-----
    "breed_hound":["661","skill"],
    "record_skill_on_scroll":["692","skill"],
    "extract_venom_from_ratspider":["693","skill"],
    "alchemy":["690","skill"],
    "brew_healing_potion":["691","skill"],
    "brew_potion_of_death":["696","skill"],
    "turn_lead_into_gold":["697","skill"],
    "make_potion_of_slavery":["694","skill"],
    "collect_rare_elements":["695","skill"],
    "all":["0","item"],
    "detect_ability_scry":["811","skill"],
    "dispel_ability_shroud":["812","skill"],
    "advanced_meditation":["813","skill"],
    "hinder_meditation":["814","skill"],
    "notify_of_gate_jumps":["869","skill"],
    "reveal_gate_key":["868","skill"],
    "detect_gates":["861","skill"],
    "gatecraft":["860","skill"],
    "advanced_sorcery":["920","skill"],
    "trance":["921","skill"],
    "petty_thief_cookie":["294","item"],
    "hound":["295","item"],
    "giant":["292","item"],
    "faery":["293","item"],
    "crystal_orb":["290","item"],
    "cyclops":["291","item"],
    "centaur":["271","item"],
    "minotaur":["272","item"],
    "undead_cookie":["273","item"],
    "fog_cookie":["274","item"],
    "wind_cookie":["275","item"],
    "rain_cookie":["276","item"],
    "mage_menial_cookie":["277","item"],
    "giant_spider":["278","item"],
    "rat":["279","item"],
    "seize_control_of_storm":["832","skill"],
    "fog_of_death":["833","skill"],
    "lightning_bolt":["831","skill"],
    "lead":["102","item"],
    "mob_cookie":["101","item"],
    "summon_ghost_warriors":["902","skill"],
    "runes_of_evil":["903","skill"],
    "necromancy":["900","skill"],
    "transcend_death":["901","skill"],
    "banish_demon_lord":["906","skill"],
    "eating_of_the_dead":["907","skill"],
    "summon_demon_lord":["904","skill"],
    "renew_demon_bond":["905","skill"],
    "skeleton":["33","item"],
    "savage":["32","item"],
    "undead":["31","item"],
    "barbarian":["34","item"],
    "battering_ram":["60","item"],
    "catapult":["61","item"],
    "siege_tower":["62","item"],
    "ratspider_venom":["63","item"],
    "lana_bark":["64","item"],
    "avinia_leaf":["65","item"],
    "spiny_root":["66","item"],
    "farrenstone":["67","item"],
    "yew":["68","item"],
    "elfstone":["69","item"],
    "curse_noncreator_loyalty":["886","skill"],
    "save_farcast_state":["851","skill"],
    "banish_undead":["852","skill"],
    "forge_magical_armor":["884","skill"],
    "conceal_identity_of_trader":["731","skill"],
    "trade":["730","skill"],
    "find_market_for_tradegood":["733","skill"],
    "find_tradegood_for_sale":["732","skill"],
    "stealth":["630","skill"],
    "petty_thievery":["631","skill"],
    "determine_inventory_of_character":["632","skill"],
    "determine_skills_of_character":["633","skill"],
    "determine_character's_lord":["634","skill"],
    "conceal_faction":["635","skill"],
    "learn_of_richest_nearby_noble":["636","skill"],
    "torture_prisoner":["637","skill"],
    "conceal_self":["638","skill"],
    "sneak_into_structure":["639","skill"],
    "hide":["99","item"],
    "drum":["98","item"],
    "opium":["93","item"],
    "clay_pot":["95","item"],
    "woven_basket":["94","item"],
    "tax_cookie":["96","item"],
    "aura_blast":["908","skill"],
    "reverse_jump_through_gate":["872","skill"],
    "teleport":["871","skill"],
    "gold":["1","item"],
    "cloak_creator_of_artifact":["891","skill"],
    "cloak_region_of_artifact_creation":["892","skill"],
    "dispel_cloaking_from_artifact":["893","skill"],
    "forge_palantir":["894","skill"],
    "fight_to_the_death":["612","skill"],
    "construct_catapult":["613","skill"],
    "combat":["610","skill"],
    "survive_fatal_wound":["611","skill"],
    "swordplay":["616","skill"],
    "weaponsmithing":["617","skill"],
    "defense":["614","skill"],
    "archery":["615","skill"],
    "shroud_abilities_from_scry":["809","skill"],
    "tap_health_for_aura":["808","skill"],
    "appear_common":["803","skill"],
    "perform_common_tasks_for_gold":["802","skill"],
    "meditate":["801","skill"],
    "magic":["800","skill"],
    "construct_siege_tower":["681","skill"],
    "construction":["680","skill"],
    "heal":["805","skill"],
    "stone_quarrying":["682","skill"],
    "pirate":["24","item"],
    "elf":["25","item"],
    "spirit":["26","item"],
    "swordsman":["20","item"],
    "crossbowman":["21","item"],
    "elite_archer":["22","item"],
    "angry_peasant":["23","item"],
    "wolf":["289","item"],
    "gorgon":["288","item"],
    "skull_of_bastrestric":["403","item"],
    "crown_of_prosperity":["402","item"],
    "imperial_throne":["401","item"],
    "giant_bird":["281","item"],
    "lion":["280","item"],
    "bandit":["283","item"],
    "giant_lizard":["282","item"],
    "harpie":["285","item"],
    "chimera":["284","item"],
    "orc":["287","item"],
    "dragon":["286","item"],
    "rally_peasant_mob":["674","skill"],
    "incite_mob_violence":["675","skill"],
    "train_angry_peasants":["676","skill"],
    "persuasion":["670","skill"],
    "bribe_noble":["671","skill"],
    "persuade_oathbound_noble":["672","skill"],
    "raise_peasant_mob":["673","skill"],
    "pitch":["261","item"],
    "flotsam":["59","item"],
    "nazgul":["55","item"],
    "winged_horse":["54","item"],
    "wild_horse":["51","item"],
    "warmount":["53","item"],
    "riding_horse":["52","item"],
    "renew_storm_strength":["829","skill"],
    "dissipate_storm":["828","skill"],
    "summon_rain":["825","skill"],
    "summon_wind":["824","skill"],
    "direct_storm":["827","skill"],
    "summon_fog":["826","skill"],
    "fierce_wind":["821","skill"],
    "weather_magic":["820","skill"],
    "scribe_weather_symbols":["823","skill"],
    "bind_storm_to_ship":["822","skill"],
    "collect_rare_foliage":["704","skill"],
    "harvest_mallorn_wood":["705","skill"],
    "harvest_opium":["706","skill"],
    "improve_opium_production":["707","skill"],
    "forestry":["700","skill"],
    "construct_battering_ram":["701","skill"],
    "harvest_lumber":["702","skill"],
    "harvest_yew":["703","skill"],
    "mithril":["82","item"],
    "gate_crystal":["83","item"],
    "leather":["80","item"],
    "ratspider":["81","item"],
    "fish":["87","item"],
    "blank_scroll":["84","item"],
    "crossbow":["85","item"],
    "absorb_aura_blast":["919","skill"],
    "teleport_items":["922","skill"],
    "locate_character":["847","skill"],
    "remove_magical_barrier":["846","skill"],
    "create_magical_barrier":["845","skill"],
    "dispel_location_shroud":["844","skill"],
    "shroud_location_from_magical_scry":["843","skill"],
    "ciphered_writing_of_areth-pirn":["842","skill"],
    "scry_location":["841","skill"],
    "scrying":["840","skill"],
    "farcasting":["849","skill"],
    "detect_location_scry":["848","skill"],
    "reveal_creator_of_artifact":["887","skill"],
    "train_wild_horse_to_riding_horse":["656","skill"],
    "train_wild_horse_to_warmount":["657","skill"],
    "breed_beasts":["654","skill"],
    "catch_wild_horses":["655","skill"],
    "capture_beasts_in_battle":["652","skill"],
    "use_beasts_in_battle":["653","skill"],
    "beastmastery":["650","skill"],
    "bird_spy":["651","skill"],
    "summon_wild_men":["658","skill"],
    "persuade_wild_men_to_remain":["659","skill"],
    "preparatory_ritual":["753","skill"],
    "lay_to_rest":["752","skill"],
    "receive_vision":["751","skill"],
    "religion":["750","skill"],
    "immunity_from_vision":["756","skill"],
    "remove_blessing_from_soldiers":["755","skill"],
    "resurrect_dead_noble":["754","skill"],
    "forge_magical_bow":["885","skill"],
    "worker":["11","item"],
    "peasant":["10","item"],
    "archer":["13","item"],
    "soldier":["12","item"],
    "elite_guard":["15","item"],
    "knight":["14","item"],
    "blessed_soldier":["17","item"],
    "pikeman":["16","item"],
    "sailor":["19","item"],
    "ghost_warrior":["18","item"],
    "language_of_the_ancients":["863","skill"],
    "jump_through_gate":["862","skill"],
    "unseal_gate_with_key":["865","skill"],
    "seal_gate":["864","skill"],
    "forcefully_unseal_gate":["867","skill"],
    "notify_if_gate_unsealed":["866","skill"],
    "mine_gold":["722","skill"],
    "mine_mithril":["723","skill"],
    "mining":["720","skill"],
    "mine_iron":["721","skill"],
    "forge_magical_weapon":["883","skill"],
    "arcane_symbols":["882","skill"],
    "forge_auraculum":["881","skill"],
    "artifact_construction":["880","skill"],
    "sailing":["601","skill"],
    "shipcraft":["600","skill"],
    "fishing":["603","skill"],
    "shipbuilding":["602","skill"],
    "destroy_artifact":["889","skill"],
    "reveal_where_artifact_was_created":["888","skill"],
    "wood":["77","item"],
    "ox":["76","item"],
    "pike":["75","item"],
    "longsword":["74","item"],
    "plate_armor":["73","item"],
    "longbow":["72","item"],
    "pretus_bones":["71","item"],
    "mallorn_wood":["70","item"],
    "iron":["79","item"],
    "stone":["78","item"],
    "reveal_abilities_of_another_mage":["807","skill"],
    "modern_magic_script":["806","skill"],
    "view_current_aura_level_of_others":["804","skill"]
    }
    # END OF TRANSCRIPTION DICT







def processstring(astring):
    """processes a string"""
    print astring+" --> ",
    alist = astring.split(" ")
    newlist = []


    for item in alist:
        print "processing item: "+str(item)+" ",
        if item in preprocessordict.keys():
                print "is in dict."
                # the skill is either researched. No use needed.
                newlist.append(preprocessordict[item][0])
        else:
            print "is not in dict."
            newlist.append(item)


    for item in newlist:
        print item,
    print

# Hmm. Orders need to be parsed. So an order checker is in place.


if __name__ == '__main__':

    recompile_preprocessdict_from_reprocess = 0
    if recompile_preprocessdict_from_reprocess == 1:
        for key in reprocessordict:
            print "\""+reprocessordict[key][0]+"\":[\""+key+"\",\""+reprocessordict[key][1]+"\"],"

            


    

    simpletesting = 0
    if simpletesting == 1:
        processstring("Hello")
        processstring("give 9394 peasant all")
        processstring("use receive_vision 3849")
        processstring("study stealth 10")
        processstring("use conceal_self 1")
        processstring("use fight_to_the_death 1")
        processstring("Mouth_of_the_Devourer")
        processstring("study breed_hound")
        print str(preprocessordict["breed_hound"])




    manual = 0
    if manual == 1:
        for i in range(0,1000):
            if str(i) in reprocessordict:
                print reprocessordict[str(i)][1]+" "+str(i)+" "+reprocessordict[str(i)][0]
        

    compile_aliasorderlist = 0
    if compile_aliasorderlist == 1:
        # This section is supposed to work with custom alliance scripts that manage templates, reports, etc
        
        orderaliaslist = ['accept', 'admit', 'attack', 'banner', 'behind', 'board', 'bribe', 'breed', 'build', 'buy', 'catch', 'claim', 'collect', 'contact', 'decree', 'default', 'defend', 'die', 'drop', 'email', 'execute', 'exhume', 'explore', 'fee', 'ferry', 'fish', 'flag', 'fly', 'forget', 'form', 'format', 'garrison', 'get', 'give', 'guard', 'honor', 'hostile', 'improve', 'lore', 'make', 'message', 'move', 'name', 'neutral', 'notab', 'oath', 'password', 'pay', 'pillage', 'players', 'pledge', 'post', 'press', 'promote', 'public', 'quarry', 'quest', 'quit', 'raze', 'recruit', 'repair', 'research', 'resend', 'rumor', 'sail', 'seek', 'sell', 'sneak', 'stack', 'stop', 'study', 'surrender', 'take', 'terrorize', 'train', 'ungarrison', 'unload', 'unstack', 'use', 'vis_email', 'wait']
        # print len(orderaliaslist)
        waitcommandlist = ['clear', 'day', 'turn', 'ferry', 'flag', 'fog', 'item', 'loc', 'not', 'owner', 'rain', 'ship', 'shiploc', 'stack', 'time', 'top', 'unit', 'wind']
        directionslist = ['n','north','s','south','w','west','e','east','in','out']
        nobleslist = ['Boduognat', 'Bullywug', 'Beowulf', 'Empirius', 'Satyr', 'Rudbert', 'Bigstick', 'Myka_Vykos', 'Skullthumper', 'Seven_fingers', 'Igor_the_Bonegrinder', 'Nepomucenos', 'Paolo_Giovanni', 'Mud', 'Frikky', 'Cain', 'Snake', 'Jinx', 'Didude', 'Banana_Nut', 'Dronte', 'Slook', 'Dreg', 'Hammer', 'Eternal', 'Bomber', 'Sloth', 'Glutony', 'Envy', 'Pride', 'Wrath', 'Passage', 'Rebellion', 'Bastard', 'Greed', 'Lust', 'Rakharo', 'Jhogo', 'Jaehaerys_Targaryen', 'Rhaegar_Targaryen', 'Strong_Belwas', 'Aerys_Targaryen', 'Drogo', 'Jorah_Mormont', 'Arstan_Whitebeard', 'Jaehaerys_Targaryen', 'Daenarys_Targaryen', 'Aegon_Targaryen', 'Rhaenys_Targaryen', 'Aemon_Targaryen', 'Jhiqui', 'Viserys_Targaryen', 'Illyrio_Mopatis', 'King_Bockor', 'Grunk', 'Braaggh', 'Korgul', 'Scrag', 'Drakk', 'Mord', 'Karash', 'Karak', 'Murk', 'Oomph', 'Claw', 'Gruumsh', 'Krusk', 'Azuul', 'Harhar', 'Ubada', 'Munguk_the_giant', 'Red_Kraken', 'Primal_Freegrazer', 'Yoshi_the_Gulper', 'Nightfury', 'Wolftail', 'Mighty_Wings', 'Doom_Dreadnaught', 'White_Spear', 'Horror_Show', 'Mouth_of_the_Devourer', 'Hard_Boiled', 'Cat_of_Purr', 'Black_Fangs', 'Feathersnake', 'Silent_Paw', 'Yu-Yu_the_Gloater', 'Totemizer', 'Dangerlight', 'Dread_Frog', 'Painful_Death', 'Souldrinker', 'Grim_Reaper', 'Pestilence', 'Famine', 'Conquest', "Satan's_Little_Helper_II", 'War', "Satan's_Little_Helper", 'Joris', 'Piet', 'David_Livingstone', 'Walking_Death', 'Ethan_Venge', 'Drago_Spatz', 'Sgt_York', 'Karl_Meckler', 'Jason_West', 'Carey_Bridger', 'John_Builder', 'Harlan_Brenach', 'Billy_Mack', 'Joe_Fish', 'Sgt_Rock', 'Jim_Nance', 'Yancy_James', 'Ziggy_Stardust', 'Jorj', 'Godzilla', 'Klaus', 'Jimmy_Dean', 'Emissary', 'Pen', 'Xen', 'Dude', 'Howl', 'Hasan', 'Talin', 'Full_of_Hot_Air', 'Seasick_dillo', 'Quadruplet_One', 'Stargazer', 'Clawer', 'Blizzard_Blown', 'Hairy_Screamer', 'Priodontes', 'Yepes_Mulita', 'Scout', 'Cold_Air_Blows_Hot', 'Bedazzled', 'Howler', 'Pichi_Reborn', 'Furball', 'Rory_McGrath', 'Alfred_Brehm', 'Lakshmi_Bai', 'Selma_Blair', 'Fafner_Pendragon', 'Walter_Raleigh', 'Arthur_Wellesley', 'Konrad_Zuse', 'Bernhard_Grzimek', 'Lee_Christmas', 'Phillip_Boa', 'Francisco_Coronado']
        aliaslist = orderaliaslist
        aliaslist.extend(waitcommandlist)
        aliaslist.extend(directionslist)
        aliaslist.extend(nobleslist)
        print "Lists extended.."
        
        
        print "complete command alias list for Olympia G3:"
        print aliaslist


    
